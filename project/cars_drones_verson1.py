...
def Ant_algorithm(distmat,weight,numcity):##蚁群算法进行初始无人车路径规划
...
def outqingjian(path1,weight,numcity,coordinates,weightflag):
...
def wurenji(jilu,coordinates,weight,num_drone):#无人机路径
...
def picture(zhongjian,drflag,zonglu,dr_connect,jiaohui):
    ...
    #        if(aa[m][0]==)
            ...
            plt.plot([aa[m][0],aa[n][0]],[aa[m][1],aa[n][1]],'y')
    ...
    for i in range(numcity - 1):
        ...
        plt.plot([cc[m][0], cc[n][0]], [cc[m][1], cc[n][1]], 'k-')

    plt.plot([cc[bestpath[0]][0], cc[n][0]], [cc[bestpath[0]][1], cc[n][1]], 'k-')
    ...
    for k in range(numcity2-1):
        ...
        plt.plot([zz[m][0],zz[n][0]],[zz[m][1],zz[n][1]],'k')
    
    plt.plot([zz[bestpath2[0]][0],zz[n][0]],[zz[bestpath2[0]][1],zz[n][1]],'k')
    ax=plt.gca()
    ax.set_title("vrp2")
    ax.set_xlabel('X axis')
    ax.set_ylabel('Y_axis')
...
[	18	,	18	]	]
    ...
    #print coordinates
    #print weight
    weightflag = [-1 for x in range(0, num_city)] #重件点与轻件点标志
    numcity = coordinates.shape[0] #城市个数
    distmat = getdistmat(coordinates) #城市的距离矩阵
    pathbest = Ant_algorithm(distmat,weight,numcity)
    ...
    if(num_drone == 0):
        ...
        mostwei_arr,weightflag,jilu,zonglu,zhongwe=outqingjian(path1,weight,numcity,coordinates,weightflag)#mostwei_arr为分割好的每段重件点，
        #if (num_drone != 0):
        drflag,dr_connect,jiaohui,first_drone=wurenji(jilu,coordinates,weight,num_drone)
        ...
        ppp = int(raw_input())
        if (ppp == 1):
            dongtai.dongtai(mostwei_arr,jilu,coordinates,speed_car,weight,maxcarweight,num_drone)
    #print drone_road
        '''
